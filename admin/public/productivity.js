...
function connectionBadge(){
  ...
  $('connection-select').value=c.id;document.querySelector('.local-tag').textContent=environments[c.environment]+(c.readOnly?' · ТОЛЬКО ЧТЕНИЕ':'');
  ...
  document.querySelector('.query-warning').textContent=c.readOnly?'Режим чтения: доступны таблицы, схемы, экспорт и мониторинг. Произвольный SQL и изменения отключены.':`SQL выполняется на «${c.name}» от имени ${c.user}. Изменения применяются сразу. Для UPDATE и DELETE проверяйте WHERE.`;
}
