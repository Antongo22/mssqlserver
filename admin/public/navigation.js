...
restoreButton.onclick=safe(async()=>{const saved=storage.read('studio.workspace.v1',null);if(!saved)throw new Error('Нет сохранённого рабочего места.');requireCleanGrid();showWorkspace('database');restoringWorkspace=true;try{if(state.connection!==saved.connection)await switchConnection(saved.connection);await loadDatabases(saved.database);if(saved.table){const t=state.tables.find(t=>t.schema===saved.table.schema&&t.name===saved.table.name);if(t){tab('tables');await openTable(t);}}}finally{restoringWorkspace=false;}});
renderNavigation();
...
$('modal-body').addEventListener('click',e=>{
  ...
  modal('Изменить подключение',field('name','Название',c.name)+selectField('environment','Окружение',[{value:'development',label:'Разработка'},{value:'test',label:'Тестирование'},{value:'production',label:'Рабочий сервер'}],c.environment)+`<label class="check-inline"><input type="checkbox" name="readOnly" ${c.readOnly?'checked':''}> Только чтение (без выполнения SQL)</label>`+(c.id==='local'?'<p class="muted">Адрес и учётные данные встроенного сервера задаются в Compose.</p>':field('server','Адрес',c.server)+field('port','Порт',String(c.port),'number')+field('user','Логин',c.user)+field('password','Новый пароль (пусто — сохранить текущий)','','password')+field('backupPath','Общий каталог бекапов',c.backupPath||'')+`<label class="check-inline"><input type="checkbox" name="trust" ${c.trustServerCertificate?'checked':''}> Доверять сертификату</label>`),async form=>{
    const v=formValues(form);await api('/api/connections/'+c.id,{method:'PATCH',body:{...v,readOnly:v.readOnly==='on',...(c.id==='local'?{}:{port:Number(v.port),trustServerCertificate:v.trust==='on'})}});await refreshConnections();if(c.id===state.connection){const database=state.database;state.generation++;state.database=null;state.table=null;state.data=null;await loadDatabases(database);}notice('Подключение обновлено.');
