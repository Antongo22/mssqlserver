  ...
  function addCard() {
    ...
    function updateLocals() {
      ...
      mappings.querySelectorAll('[data-map]').forEach(select => {
        ...
        select.innerHTML = '<option value="">Выберите столбец новой таблицы</option>' + locals.map(l => `<option value="${esc(l.id)}" ${!l.name ? 'disabled' : ''}>${esc(l.name || '(без имени)')} · ${esc(l.type)}</option>`).join('');
    ...
    function renderMappings() {
      ...
      if (!key) { mappings.innerHTML = '<p class="muted">Выберите родительский PK.</p>'; preview(); return; }
